...
class Network:
    def __init__(self, sizes):  # , cost , smallWeightInit=True):
        ...
        self.weights = [
            numpy.random.randn(input_dim + 1, output_dim)
            for output_dim, input_dim in zip(sizes[1:], sizes[:-1])
        ]
    ...
    def backpropagation(self, outputs, delta):
        ...
        for i in range(1, self.layerNum):
            ...
            nabla = [
                numpy.concatenate((outputs[-i - 1].transpose(), ones), axis=0).dot(
                    delta
                )
            ] + nabla
            ...
            delta = delta.dot(self.weights[-i][:-1].transpose()) * sigmoid_prime(
                outputs[-i - 1]
            )
